...
# K = 2
ProClus_clusters_k2<- ProClus(df_noCorr,k=2,d=70)
k2_cluster1<-ProClus_clusters_k2[[1]]$objects
k2_cluster2<-ProClus_clusters_k2[[2]]$objects


# K = 4
ProClus_clusters_k4 <- ProClus(df_noCorr,k=4,d = 20)
...
for(i in k2_cluster1){
  ...
  labels_proclus_k2[i] <- 1

}
for(i in k2_cluster2){

  labels_proclus_k2[i]<- 2
...
for(i in k4_cluster1){
  ...
  labels_proclus_k4[i] <- 1

}
for(i in k4_cluster2){

  labels_proclus_k4[i]<- 2

}

for(i in k4_cluster3){

  labels_proclus_k4[i]<- 3

}
for(i in k4_cluster4){

  labels_proclus_k4[i]<- 4

}
```

dataframe for each cluster since it has different dimensions
...
df_proclus_k4_cluster1 <- df_noCorr[k4_cluster1,which(ProClus_clusters_k4[[1]][["subspace"]] == TRUE)]
df_proclus_k4_cluster2 <- df_noCorr[k4_cluster2,which(ProClus_clusters_k4[[2]][["subspace"]] == TRUE)]
df_proclus_k4_cluster3 <- df_noCorr[k4_cluster3,which(ProClus_clusters_k4[[3]][["subspace"]] == TRUE)]
df_proclus_k4_cluster4 <- df_noCorr[k4_cluster4,which(ProClus_clusters_k4[[4]][["subspace"]] == TRUE)]


```

Creating dataframe with cluster label, scaled and all Features

```{r}

df_allF_p2_labeled<- df_allF%>%
  mutate(label = labels_proclus_k2)
df_allF_scaled_p2_labeled<-df_allF_scaled%>%
  mutate(label = labels_proclus_k2)

df_allF_p4_labeled<-df_allF%>%
  mutate(label = labels_proclus_k4)
df_allF_scaled_p4_labeled<-df_allF_scaled%>%
  mutate(label = labels_proclus_k4)
...
```{r}
cluster_features_scaled<-data.frame(df_allF_scaled_p2_labeled%>%
                               group_by(label)%>%
                               summarise_each(mean))
cluster_features<-data.frame(df_allF_p2_labeled%>%
                               group_by(label)%>%
                               summarise_each(mean))
...
set.seed(123)
df_string_labeled<-df_allF_scaled_p2_labeled
#df_string_labeled$label <- as.character(df_string_labeled$label)
df_string_labeled$label<-sub("^","Type ",df_allF_scaled_p2_labeled$label) #Adding "Type" to each cluster number to make it                                                             #non numeric
...
```{r}
df_allF_p2_labeled$label <- as.factor(df_allF_p2_labeled$label)
...
x <- df_allF_p2_labeled[,1:77]
y <- df_allF_p2_labeled[,78]
...
#set.seed(123)
custom <- train(label~., data=df_allF_p2_labeled, method=customRF,  tuneGrid=tunegrid, trControl=control)
#summary(custom)
plot(custom)
print(custom)
```
...
```{r}
data.rf <- randomForest(formula= label ~ ., data=df_allF_p2_labeled, importance=TRUE,ntree=1000,mtry=12,
                        proximity=TRUE)
