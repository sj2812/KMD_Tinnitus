...
tune_l_sparsity<- data.frame(k = numeric(),dim = numeric(),sparsity = numeric(),proj = numeric())
tune_l_sse<-vector("list",1)
j<-1
...
for (k in seq(10,30)) {
  ...
  for (i in seq(20,70)) {
    ...
    tune_l_sparsity[j,1] <- k
    tune_l_sparsity[j,2] <- i
    tune_l_sparsity[j,3] <- orclus.res.k2$sparsity.coefficient
    tune_l_sparsity[j,4] <- sum(orclus.res.k2$within.projens)
    j <- j+1
  }
}
