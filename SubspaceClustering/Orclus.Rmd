...
tune_l_sparsity<-vector("list",3)
tune_l_sse<-vector("list",1)
j<-0
for (i in seq(20,60,by = 5)) {
  
  orclus.res.k2 <- orclus(x = subspace_data,k=4
                        ,l = i, k0 = 20)
  tune_l_sparsity[[1]]<-i
  tune_l_sparsity[[2]]<-orclus.res.k2$sparsity.coefficient
  tune_l_sparsity[[3]]<-sum(orclus.res.k2$within.projens)
  j<-j+1
}
