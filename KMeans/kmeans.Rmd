...
df_labeled_Kmeans<-df[,-1]%>%
  mutate(label = klabels)
df_labeled_Kmeans%>%head

df_scaled_labeled_Kmeans<-df_scaled_allF%>%
  mutate(label = klabels)
df_scaled_labeled_Kmeans%>%head

```

Grouping the data based on the assigned labels and splitting into one data frame for each group.(extra step)
```{r}
df_scaled_grouped_Kmeans<-df_scaled_labeled_Kmeans%>%
  group_by(label)
df_split<-group_split(df_scaled_grouped_Kmeans)
...
```{r}
cluster_features<-data.frame(df_scaled_labeled_Kmeans%>%
                               group_by(label)%>%
                               summarise_each(mean))
