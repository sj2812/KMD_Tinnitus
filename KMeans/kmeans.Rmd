...
fviz_nbclust(df_noCorr_scaled, kmeans, method = "silhouette", k.max = 8)
...
kmclust_withinss <- function(k){
  kmclust <- kmeans(df_noCorr_scaled,k,nstart=50,iter.max = 10 )
...
image(t(df_noCorr_scaled)[, nrow(df_noCorr_scaled):1],
...
for(k in kcenters){
  ...
  set.seed(123)
  kmclust <- kmeans(df_noCorr_scaled,k, nstart = 50, 10)
  
  image(t(df_noCorr_scaled)[, order(kmclust$cluster)], 
        col = hcl.colors(20, "Spectral", rev = TRUE), 
        yaxt = "n", xaxt = "n",
...
#heatmap.2(df_noCorr_scaled, kmclust, col=hmcols, trace= "none", main = paste("Clustered Data k=",k))
...
kmclust <- kmeans(df_noCorr_scaled,k,iter.max = 10)
...
for (k in kcenters){
kmclust <- kmeans(df_noCorr_scaled,k, nstart=50,iter.max = 15)
print(fviz_cluster(kmclust, df_noCorr_scaled, ellipse.type  = "norm", ellipse.level = 0.68, main =  paste("Clustered Data k=",k) ))
}
```
...
kmclust2 <- kmeans(df_noCorr_scaled,2, nstart=50,iter.max = 15)
kmclust4 <- kmeans(df_noCorr_scaled,4, nstart=50,iter.max = 15)

km2_labels <- kmclust2$cluster # klabels for getting labelled data
head(km2_labels, n = 50)
km4_labels <- kmclust4$cluster # klabels for getting labelled data
head(km4_labels, n = 50)
...
df_allF_km2_labeled<-df_allF%>%
  mutate(label = km2_labels)
df_allF_scaled_km2_labeled<-df_allF_scaled%>%
  mutate(label = km2_labels)

df_allF_km4_labeled<-df_allF%>%
  mutate(label = km4_labels)
df_allF_scaled_km4_labeled<-df_allF_scaled%>%
  mutate(label = km4_labels)
...
```{r}
df_grouped_km<-df_allF_km2_labeled%>%
  group_by(label)
df_split<-group_split(df_grouped_km)
...
cluster_features_scaled<-data.frame(df_allF_scaled_km2_labeled%>%
                               group_by(label)%>%
                               summarise_each(mean))
cluster_features<-data.frame(df_allF_km2_labeled%>%
                               group_by(label)%>%
                               summarise_each(mean))
...
set.seed(123)
df_string_km2_labeled<-df_allF_km2_labeled
df_string_km2_labeled$label<-sub("^","Type ",df_allF_km2_labeled$label) #Adding "Type" to each cluster number to make it                                                             #non numeric
