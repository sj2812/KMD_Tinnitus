...
#do you want to tune hierarchical clustering to get the best
...
if(tuneHierarchical)
  tunehierarchical()
```
The following chunk tunes the hyper-parameters l(final cluster specific subspaces dimension) and k0(initial number of clusters) by the “sparsity.coefficient” and the “within.projenss”measures obtained along with the clustering result where lower values for sparcity coefficientand wss are expected.
```{r OrclusTune,echo=FALSE}
...
tunegrid <- expand.grid(.mtry=c(5:7), .ntree=c(500,1000))
set.seed(123)
...
getprocluslabels<-function(numClust,avgdim){
 ...
 getorcluslabels<-function(numClust,l,k0){
  set.seed(123)
  orclus_res_k <- orclus:: orclus(df_noCorr_scaled,k = numClust,l = l, k0 = k0)
...
This chunk can be used for Configuration of different approaches. These function calls are used to get the cluster assignment of the approches.User can store the proclus result object to a file for future reuse.
```{r} 
...
if(reproduceProclusResults){
  ...
  orclusres<-getorcluslabels(4,finalSubspaceDimOrclus,initialNumberOfClustersOrclus)
  
  pckmres<-getpcakmeanslabels(4,variancePCAKmeans)
...
if(valDTProclus){
  ...
  applyrf(proclusres$labels,proclusres$numClust,1000,5,1) 
}
if(valDTOrclus){
  bestparamdf<-tuneRandomForest(orclusres$labels)
  applyrf(orclusres$labels,num,bestparamdf$ntree,bestparamdf$mtry,0)
}
if(valDTPCAKmeans){
  bestparamdf<-tuneRandomForest(pckmres$labels)
