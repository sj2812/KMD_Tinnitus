...
correlations <- function(corrval)
{
  ...
  for (i in 1:nrow(matriz_cor)){
    correlations <-  which((abs(matriz_cor[i,]) > corrval) & (matriz_cor[i,] != 1))
...
```
## User configuration for the value of correlation.
...
getprocluslabels<-function(numClust,avgdim){
  set.seed(123)
  ProClus.clusters.k<- ProClus(df_noCorr_scaled,k=numClust,d=avgdim)
 ...
 getorcluslabels<-function(numClust,l,k0){
   set.seed(123)
  library(orclus)
  orclus_res_k <- orclus(df_noCorr_scaled,k = numClust,l = l, k0 = k0)
...
```{r} 
#kmeans with configurable no. of clusters
  kmeansres<-getKmeanslabels(6)
#hierarchical with configurable no. of clusters and clustering method("ward","average","single","complete","weighted")
  hierarchicalres<-gethierarchicallabels(6,"ward")
 #hkmeans with configurable no. of clusters and clustering method("ward","average","single","complete","weighted") 
  hkmeansres<-gethKmeanslabels(4,"ward")
#Proclus with configurable no. of clusters and average dimensionality
  proclusres<-getprocluslabels(2,25)
# saving the proclus results to view them later
  save(proclusres, file = paste("data",proclusres$numClust,".RData"))
  #If you want to view results for particular number of clusters that was previously run then using the following load function with the filename data and append the number of clusters for. eg. data 3 .RData= result of proclus  when number of clusters was 3.  
  
  #load("data 2 .RData")
#orclus with configurable no. of clusters, final subspace dimension and initial number of clusters
  orclusres<-getorcluslabels(4,25,20)
  pckmres<-getpcakmeanslabels(2,0.7)
  
```

All the results can be seen after running this chunk for number of clusters 2 to 10
```{r}
