...
tunehierarchical <- function() {
  ...
  for (dist_m in dist_methods)
  {
    ...
    for (x in m)
    {
      ...
      ac <- hc$ac
      title <-
        sprintf("Distance: %s,Inter cluster Distance: %s, ac value: %f",
                dist_m,
                x,
                ac)
      pltree(hc,
...
getpcakmeanslabels<-function(numClust,variance){
  ...
  plot(cumpro[0:maxPC], xlab = "PC Numbers", ylab = "Amount of explained variance", main = paste("Score plot of      variance =", variance*100, "% at PC =",PC_num))
...
These function calls are used to get the cluster assignment of the approches.User can store the proclus result object to a file for future reuse.
```{r} 
...
```
## Output
The output in the form of decision trees and radil charts can be seen after running this chunk for number of clusters from 2 to 10
```{r}
...
As this chunk does the tuning of random forest, it may take some time
