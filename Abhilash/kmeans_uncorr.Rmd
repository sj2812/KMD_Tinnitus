...
fviz_nbclust(df_scaled_uncorr, kmeans, method = "silhouette", k.max = 8)
...
kmclust_withinss <- function(k){
  kmclust <- kmeans(df_scaled_uncorr,k,nstart=50,iter.max = 10 )
...
image(t(df_scaled_uncorr)[, nrow(df_scaled_uncorr):1],
...
for(k in kcenters){
  ...
  set.seed(123)
  kmclust <- kmeans(df_scaled_uncorr,k, nstart = 50, 10)
  
  image(t(df_scaled_uncorr)[, order(kmclust$cluster)], 
        col = hcl.colors(5, "Spectral", rev = TRUE), 
        yaxt = "n", 
        ylab = "row  order",xlab = "Heatmap values of rows", 
...
#heatmap.2(df_scaled_uncorr, kmclust, col=hmcols, trace= "none", main = paste("Clustered Data k=",k))
...
kmclust <- kmeans(df_scaled_uncorr,k,iter.max = 10)
...
for (k in kcenters){
kmclust <- kmeans(df_scaled_uncorr,k, nstart=50,iter.max = 15)
print(fviz_cluster(kmclust, df_scaled_uncorr, ellipse.type  = "norm", ellipse.level = 0.68))
}
```
...
kmclust <- kmeans(df_scaled_uncorr,4, nstart=50,iter.max = 15)
...
# km.boot <- clusterboot(df_scaled_uncorr, B=20, bootmethod="boot",
...
bootsample <- lapply(1:nrow(df_scaled_uncorr), function(i){df_scaled_uncorr[sample(df_scaled_uncorr, ncol(df_scaled_uncorr), replace = T)]})
y2
bootsample1 <- matrix(unlist(bootsam), ncol = ncol(df_scaled_uncorr), byrow =TRUE)
bootsample1
...
bootsample2 <- boot(df_scaled_uncorr,func, R=10)
bootdata <- bootsample2$t0
bootdata1 <- bootsample2$t
bootdata11 <- bootsample2$t[1,]
bootdata111 <-  matrix(unlist(bootdata11), ncol = ncol(df_scaled_uncorr), byrow =TRUE)
