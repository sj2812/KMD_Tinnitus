...
print(fviz_cluster(kmclust, df_scaled, ellipse.type  = "norm", ellipse.level = 0.68))
}
```
...
kmclust <- kmeans(df_scaled,4, nstart=50,iter.max = 15)
...
# km.boot <- clusterboot(df_scaled, B=20, bootmethod="boot",
...
bootsample <- lapply(1:nrow(df_scaled), function(i){df_scaled[sample(df_scaled, ncol(df_scaled), replace = T)]})
y2
bootsample1 <- matrix(unlist(bootsam), ncol = ncol(df_scaled), byrow =TRUE)
bootsample1
...
bootsample2 <- boot(df_scaled,func, R=10)
bootdata <- bootsample2$t0
bootdata1 <- bootsample2$t
bootdata11 <- bootsample2$t[1,]
bootdata111 <-  matrix(unlist(bootdata11), ncol = ncol(df_scaled), byrow =TRUE)
