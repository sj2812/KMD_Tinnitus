...
```{r}
#Data preprocessing
df <- read_rds('C:\\Users\\Kritika\\Documents\\SummerSem2019\\KMD Project\\KMD_Tinnitus\\190426_charite_tinnitus.rds') %>%
  arrange(.testdatum) %>%
  group_by(.jour_nr) %>%
  slice(1) %>%
  ungroup() %>%
...
```{r}
options(scipen = 999)
for(cluster in 1:nrow(cluster_features))
{
  cluster_plot_data<-rbind(feature_summary,cluster_features[cluster,-1])
  cluster_name<-paste("cluster_",cluster_features[cluster,1])
