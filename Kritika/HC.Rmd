...
Also calculaselect(-.jour_nr)%>%ting the general population summary for each feature as "feature summary".
```{r}
...
```{r}
data.rf <- randomForest(formula= label ~ ., data=df_labeled, importance=TRUE,ntree=1500,mtry=5,
                        proximity=TRUE)
