...
df_labeled$cluster<-res.hk$cluster
View(df_labeled)

cluster_features<-data.frame(df_labeled%>%
                               group_by(cluster)%>%
                               summarise_each(mean))
feature_summary<-data.frame(df_labeled[1:73]%>%
                               summarise_each(mean))
...
```
```
supervised phase to obtain the important features using random forest. This chunk does the tuning of the parameters for random forest. There are no inbuilt function to obtain the optimal number of trees i.e mtree could not be tuned using available packages so this code imbibes the parameter tuning for mtree as well and gives the best mtry and ntree parameters. mtry is the number of variables randomly sampled as candidates at each split.
```{r}
...
```{r}
data.rf <- randomForest(formula= cluster ~ ., data=df_labeled, importance=TRUE,ntree=1000,mtry=5,
                        proximity=TRUE)
