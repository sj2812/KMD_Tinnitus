...
cluster_features<-data.frame(df_allF_scaled_hk2_labeled%>%
                               group_by(label)%>%
                               summarise_each(mean))
feature_summary<-data.frame(df_allF_scaled_hk2_labeled[1:77]%>%
                               summarise_each(mean))
cluster_features4<-data.frame(df_allF_scaled_hk4_labeled%>%
                               group_by(label)%>%
                               summarise_each(mean))
feature_summary4<-data.frame(df_allF_scaled_hk4_labeled[1:77]%>%
                               summarise_each(mean))
