...
rownames( df_scaled ) <- seq_len( nrow( df_scaled ) )
library("clValid")
intern <- clValid(df_scaled, 2:6, 
              clMethods = c("hkmeans"),
              validation = "internal")
...
dunn_km = dunn(clusters = res.hk$cluster, Data=df_scaled)
dunn_km
connectivity(distance = NULL, res.hk$cluster, Data = df_scaled, neighbSize = 10, method = "euclidean")
```
...
View(df_labeled)
library(diceR)
compactness(df_scaled, df_labeled$cluster)
cluster_features<-data.frame(df_labeled%>%
                               group_by(cluster)%>%
                               summarise_each(mean))
feature_summary<-data.frame(df_labeled[1:77]%>%
                               summarise_each(mean))
...
df_string_labeled$cluster<-paste("type",df_labeled$cluster)

trctrl <- trainControl(method = "boot", number = 10)
dtree_fit <- train(cluster ~., data = df_string_labeled, method = "rpart",
