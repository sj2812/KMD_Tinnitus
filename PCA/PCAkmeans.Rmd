...
km.pc <- prcomp(df_scaled, center = TRUE)
summary(km.pc)
```
...
set.seed(123)
k=2
kmclust <- kmeans(df_PC,k, nstart=50,iter.max = 15)

klabels <- kmclust$cluster # klabels for getting labelled data
head(klabels, n = 50)
...
df_labeled_PCKmeans<-df[,-1]%>%
  mutate(label = klabels)
df_labeled_PCKmeans%>%head

df_scaled_labeled_PCKmeans<-df_scaled_allF%>%
  mutate(label = klabels)
df_scaled_labeled_PCKmeans%>%head

```

Grouping the data based on the assigned labels and splitting into one data frame for each group.(extra step)
```{r}
df_scaled_grouped_PCKmeans<-df_scaled_labeled_PCKmeans%>%
  group_by(label)
df_split<-group_split(df_scaled_grouped_PCKmeans)
...
```{r}
cluster_features<-data.frame(df_scaled_labeled_PCKmeans%>%
                               group_by(label)%>%
                               summarise_each(mean))
...
set.seed(123)
df_string_labeled<-df_labeled_PCKmeans
df_string_labeled$label<-sub("^","Type ",df_labeled_PCKmeans$label) #Adding "Type" to each cluster number to make it                                                             #non numeric
