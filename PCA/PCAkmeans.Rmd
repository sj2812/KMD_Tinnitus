...
km.pc <- prcomp(df_scaled_allF, center = TRUE)
summary(km.pc)
```

```{r}

screeplot(km.pc, type = "l", npcs = 30, main = "Screeplot of the first 30 PCs")
...
plot(cumpro[0:30], xlab = "PC Numbers", ylab = "Amount of explained variance", main = paste("Cumulative variance plot at PC =",PC_num))
...
#Creating dataset with first 17 PCs
...
df_scaled_labeled <- df_scaled_allF%>%
...
```{r}
df_scaled_grouped<-df_scaled_labeled%>%
  group_by(label)
df_split<-group_split(df_scaled_grouped)
...
cluster_features<-data.frame(df_scaled_labeled%>%
                               group_by(label)%>%
                               summarise_each(mean))
feature_summary<-data.frame(df_scaled_allF%>%
                               summarise_each(mean))
